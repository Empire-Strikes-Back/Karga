...
(defonce program-data-dirpath
  (or
   (some-> (System/getenv "KARGA_PATH")
           (.replaceFirst "^~" (System/getProperty "user.home")))
   (.getCanonicalPath ^File (clojure.java.io/file (System/getProperty "user.home") ".Karga"))))

(defonce program-db-dirpath (.getCanonicalPath ^File (clojure.java.io/file program-data-dirpath "db")))

(defonce state-file-filepath (.getCanonicalPath ^File (clojure.java.io/file program-data-dirpath "Karga.edn")))
...
(defonce resize|m (mult resize|))

#_(defonce *ns (find-ns 'Karga.main))

(def ^:const jframe-title "no, i for one, i celebrate Mando's success - because it is my success as well - hell, even i am rich")
              ...
              (.add (doto (JMenuItem.)
                      ...
                      (.addActionListener
                       (on-menubar-item (fn [_ event]
                                          (put! tabs| {:op :tab :tab-name :apricots})
...
(defn -main
  [& args]
  (println "you had you shot, dust breather, but you failed - no pucks for you - now get outta here")
  (println "i dont want my next job")
  (println "Kuiil has spoken")

  #_(alter-var-root #'*ns* (constantly (find-ns 'Karga.main)))
     ...
     (reify Runnable
       (run [_]
            ...
            (let [jtabbed-pane (JTabbedPane.)
                  tabs {:apricots (JPanel.)
                        :B12 (JPanel.)
                        :salt (JPanel.)
                        :bread (Karga.bread/create-ui {:jframe jframe
                                                       :resize| resize|})
                        :raisins (JPanel.)}]
              ...
              #_(doto ^JPanel (:bread tabs)
                  (.setLayout (MigLayout. "insets 10"
                                          "[grow,shrink,fill]"
                                          "[grow,shrink,fill]")))

              (Karga.bread/process {:jpanel-tab (:bread tabs)
              ...
              (discover-process
               {:jpanel-tab (:apricots tabs)
                :ops| ops|
                :gamesA gamesA
                :gameA gameA
                :stateA stateA})

              #_(.add ^JPanel (:bread tabs) (JTextField. "Word") "id Word, pos 50%-Word.w 50%-Word.h" #_"dock center,width 100 :100%:100%")
            ...
            (alter-var-root #'Karga.main/jroot-panel (constantly jroot-panel))
